...
export const dbCreateForumTopic = (topic: Omit<ForumTopic, 'id' | 'repliesCount' | 'date'>): Promise<ForumTopic[]> => {
    const newTopic = { ...topic, id: Date.now(), repliesCount: 0, date: new Date().toISOString() };
    forumTopics.unshift(newTopic);
    return resolve(forumTopics);
};

export const dbMarkNotificationRead = (notifId: number): Promise<void> => {
